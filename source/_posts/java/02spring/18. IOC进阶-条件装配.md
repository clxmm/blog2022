...
如果现在直接运行 `TavernProfileApplication` 的 `main` 方法，控制台中不会打印 zs 和 ls 
...
tavernConfiguration
org.clxmm.configuration.b_profile.component.Boss
org.clxmm.configuration.b_profile.component.Bar
...
这样子一写，再重新运行 `main` 方法，果然控制台就打印 sz 和 ls 了！
...
这样配置好之后，在 `main` 方法中改回原来的构造方法传入配置类的形式，运行，控制台仍然会打印 sz 和 ls 。

修改传入的 jvm 参数，将 city 改成 **wilderness** ，重新运行 `main` 方法，发现控制台不再打印 sz 和 ls ，说明使用 jvm 命令行参数也可以控制 profile 。
...
org.springframework.context.event.internalEventListenerFactory
tavernConfiguration
org.clxmm.configuration.c_conditional.component.Boss
org.clxmm.configuration.c_conditional.config.BartenderConfiguration
zs
ls
org.clxmm.configuration.c_conditional.component.Bar
...
//    @Conditional(ExistBossCondition.class)
    @ConditionalOnBean(beanNames = "org.clxmm.configuration.c_conditional.component.Boss")
    public Bar bbbar() {
        return new Bar();
    }
}
