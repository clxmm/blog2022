---
title: 27. IOC高级-BeanPostProcessor的扩展及作用时机.md
toc: true
tags: spring
categories: 
    - [java]
...
上一章，咱对 BeanPostProcessor 有一个大体的认识，也做了几个练习和探究。这一章我们来研究 BeanPostProcessor 的扩展，内容难度会更高，小伙伴们一定要认真记笔记、做练习，多思考多总结呀。
...
![](/img/spring/202210/27beanpostProcessor.png)

这些接口，各自扩展出来的功能也都不同，下面我们逐个来研究。
...
![](/img/spring/202210/27beanpostProcessor1.png)
...
![](/img/spring/202210/27beanpostProcessor2.png)
...
```xml
    <bean id="ball2" class="org.clxmm.postprocessor.e_instantiation.bean.Ball"></bean>
...
前面在 bean 的生命周期中，我们有写过一个全生命周期的 Pen,我们可以拿来利用一下。

#### 3.1.1 声明Bean

虽说是用那个 `Pen` ，不过在这个 Demo 中我们只需要它的销毁方法：（此处只声明了 `@PreDestroy` 与 `DisposableBean` ）
...
public class DestructionPostProcessorApplication {
    public static void main(String[] args) {
        AnnotationConfigApplicationContext context = 
          new AnnotationConfigApplicationContext("org.clxmm.postprocessor.f_destruction");
...
在前面的 25 章第 3 节，咱有学习 `BeanDefinition` 的合并，这个 `BeanDefinition` 合并的过程，在后置处理器中也有对应的拦截处理。
...
public class MergeDefinitionPostProcessorApplication {
    ...
    public static void main(String[] args) {
        AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(
                "org.clxmm.postprocessor.g_mergedefinition");
        ctx.close();
    }
}
...
- `BeanPostProcessor` 的扩展都有哪些？
- `InstantiationAwareBeanPostProcessor` 扩展了哪些回调时机？
- 整理现有已经学过的 bean 的生命周期中的各种回调时机，并编写一个综合小练习，加深对生命周期的理解和印象。

【 SpringFramework 中关于 bean 的后置处理器就差不多这些，下面还有另外一种后置处理器，它是针对 `BeanDefinition` 的：**`BeanFactoryPostProcessor`**】
