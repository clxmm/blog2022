...
![](./img/2023/10/java-46-2-84686f10.png)
...
![](./img/2023/10/java-47-3-a63c3de1.png)
...
![](./img/2023/10/java-48-4-c4285c13.png)
...
![](./img/2023/10/java-51-8-ee90e2f2.png)
...
![](./img/2023/10/java-52-7-5b9136ff.png)
...
### 6.`ThreadLocalMap.set()`详解

####  1.`ThreadLocalMap.set()`原理图解
...
![](./img/2023/10/java-56-12-7f276023.png)
...
![](./img/2023/10/java-57-13-130fe75c.png)
...
![](./img/2023/10/java-59-14-28205930.png)
...
![](./img/2023/10/java-60-view.png)

向后遍历过程中，如果没有找到相同 key 值的 Entry 数据：

![](./img/2023/10/java-58-15-e6795086.png)
...
- 查找过程中，碰到桶中`Entry=null`的情况，直接使用

接着就是执行`for`循环遍历，向后查找，我们先看下`nextIndex()`、`prevIndex()`方法实现：

![](./img/2023/10/java-62-17-ce02241d.png)
...
2. 如果`key`值对应的桶中`Entry`数据不为空
   1. 2.1 如果`k = key`，说明当前`set`操作是一个替换操作，做替换逻辑，直接返回
   2. 2.2 如果`key = null`，说明当前桶位置的`Entry`是过期数据，执行`replaceStaleEntry()`方法(核心方法)，然后返回
3. `for`循环执行完毕，继续往下执行说明向后迭代的过程中遇到了`entry`为`null`的情况
   1. 3.1 在`Entry`为`null`的桶中创建一个新的`Entry`对象
   2. 3.2 执行`++size`操作
4. 调用`cleanSomeSlots()`做一次启发式清理工作，清理散列数组中`Entry`的`key`过期的数据
   1. 4.1 如果清理工作完成后，未清理到任何数据，且`size`超过了阈值(数组长度的 2/3)，进行`rehash()`操作
   2. 4.2 `rehash()`中会先进行一轮探测式清理，清理过期`key`，清理完成后如果**size >= threshold - threshold / 4**，就会执行真正的扩容逻辑(扩容逻辑往后看)
...
### 7.`ThreadLocalMap`过期 key 的探测式清理流程
...
![](./img/2023/10/java-63-18-4ae47cc3.png)
...
![](./img/2023/10/java-64-19-a9cb9d21.png)
...
![](./img/2023/10/java-69-24-ec7f7610.png)

接着看看具体的`resize()`方法，为了方便演示，我们以`oldTab.len=8`来举例：

![](./img/2023/10/java-70-25-5e3dc525.png)
...
#### 1. `ThreadLocalMap.get()`图解

**第一种情况：** 通过查找`key`值计算出散列表中`slot`位置，然后该`slot`位置中的`Entry.key`和查找的`key`一致，则直接返回：

![](./img/2023/10/java-71-26-ff0553de.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![](./img/2023/10/java-72-27-9c78c2a2.png)
...
![](./img/2023/10/java-73-28-ea7d5196.png)

#### 2.`ThreadLocalMap.get()`源码详解
...
### 10.`ThreadLocalMap`过期 key 的启发式清理流程
...
![](./img/2023/10/java-74-29-c87cfab8.png)
...
![](./img/2023/10/java-75-31-742c0f94.png)
